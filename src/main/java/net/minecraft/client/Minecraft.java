...
import moshi.blossom.event.impl.KeyEvent;
...
public class Minecraft implements IThreadListener, IPlayerUsage
{
    ...
    public void runTick() throws IOException
    {
        ...
        if (this.currentScreen == null || this.currentScreen.allowUserInput)
        {
            ...
            {
                while (this.gameSettings.keyBindAttack.isPressed() && !(new MouseTickEvent(MouseTickEvent.Type.LEFT)).call().isCanceled())
                {
                    this.clickMouse();
                }

                while (this.gameSettings.keyBindUseItem.isPressed() && !(new MouseTickEvent(MouseTickEvent.Type.RIGHT)).call().isCanceled())
                {
                    this.rightClickMouse();
                }

                while (this.gameSettings.keyBindPickBlock.isPressed() && !(new MouseTickEvent(MouseTickEvent.Type.MIDDLE)).call().isCanceled())
                {
                    this.middleClickMouse();
                }
            }

            if (this.gameSettings.keyBindUseItem.isKeyDown() && this.rightClickDelayTimer == 0 && !this.thePlayer.isUsingItem() && !(new MouseTickEvent(MouseTickEvent.Type.RIGHT_HOLD)).call().isCanceled())
            {
                this.rightClickMouse();
            }
