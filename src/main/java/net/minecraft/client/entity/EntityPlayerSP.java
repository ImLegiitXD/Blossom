...
public class EntityPlayerSP extends AbstractClientPlayer
{
    ...
    public void onUpdate()
    {
        if (this.worldObj.isBlockLoaded(new BlockPos(this.posX, 0.0D, this.posZ))) {
            ...
            (new UpdateEvent(this)).call();
            this.mc.thePlayer.rotationPitchHead = this.mc.thePlayer.rotationPitch;
    ...
    public void onUpdateWalkingPlayer() {
        this.offGroundTicks = !this.onGround ? (this.offGroundTicks + 1) : 0;

        EntityActionEvent actionEvent = new EntityActionEvent(isSprinting(), isSneaking());
        actionEvent.call();

        boolean sprinting = actionEvent.isSprinting();
        ...
        boolean sneaking = actionEvent.isSneaking();
        ...
        if (isCurrentViewEntity()) {
            ...
            double deltaX = event.getX() - this.lastReportedPosX;
            double deltaY = event.getY() - this.lastReportedPosY;
            double deltaZ = event.getZ() - this.lastReportedPosZ;
            double deltaYaw = event.getYaw() - this.lastReportedYaw;
            double deltaPitch = event.getPitch() - this.lastReportedPitch;
            ...
            if (this.ridingEntity == null) {
                if ((positionChanged && rotationChanged) || event.force06) {
   ...
   public void sendChatMessage(String message) {
      if ((new ChatEvent(message)).call().isCanceled())
      return;
    ...
    public void onLivingUpdate()
    {
        ...
        if (isUsingItem() && !isRiding() && !(new SlowDownEvent()).call().isCanceled()) {
    ...
    public void moveEntity(double x, double y, double z) {
             MoveEvent moveEvent = new MoveEvent(x, y, z, this);
             moveEvent.call();
