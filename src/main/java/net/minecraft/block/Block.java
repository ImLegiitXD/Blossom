...
import moshi.blossom.module.impl.render.XRay;
...
public class Block
{
    ...
    public void addCollisionBoxesToList(World worldIn, BlockPos pos, IBlockState state, AxisAlignedBB mask, List<AxisAlignedBB> list, Entity collidingEntity) {
        CollideEvent event = new CollideEvent(pos, state, getCollisionBoundingBox(worldIn, pos, state), list);
           if (collidingEntity == (Minecraft.getMinecraft()).thePlayer)
               event.call();
          if (event.isCanceled()) {
                 return;
          }
          AxisAlignedBB axisalignedbb = event.getAlignedBB();

           if (axisalignedbb != null && mask.intersectsWith(axisalignedbb)) {
               event.getAxisList().add(axisalignedbb);
           }
    }
    ...
    public EnumWorldBlockLayer getBlockLayer() {
         if (ModManager.getMod("XRay").isToggled()) {
             return ((XRay)ModManager.getMod("XRay")).blockIDs.contains(Integer.valueOf(getIdFromBlock(this))) ? EnumWorldBlockLayer.SOLID : EnumWorldBlockLayer.TRANSLUCENT;
                  }
