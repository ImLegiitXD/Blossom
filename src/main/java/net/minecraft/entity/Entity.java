...
public abstract class Entity implements ICommandSender
{
    ...
    public void moveEntity(double x, double y, double z)
    {
        ...
        else
        {
            ...
            boolean flag = (this.onGround && (isSneaking() || event.isForceSafeWalk()) && this instanceof EntityPlayer);
            ...
            if (flag)
            {
                ...
                for (d6 = 0.05D; x != 0.0D && this.worldObj.getCollidingBoundingBoxes(this, this.getEntityBoundingBox().offset(x, -1.0D, 0.0D)).isEmpty(); d3 = x)
                {
                    if (x < d6 && x >= -d6) {
                        ...
                        //ChatUtil.print("XD"); NOSE PORQUE ESTO LO AÑADIERON
                        x = 0.0D;
                    }
                    else if (x > 0.0D)
                    {
    ...
    public void moveFlying(float strafe, float forward, float friction) {
             FlyingEvent event = new FlyingEvent(this.rotationYaw, forward, strafe);
