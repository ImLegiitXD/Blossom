...
extends Command {
    ...
    public String run(String[] args) {
        if (args.length == 0) return ChatUtil.printInfo("usage: .config <list/save/load/delete> <name>");
        ...
        if (args.length == 2) {
            switch (args[0]) {
                case "save":
                return ChatUtil.printInfo(Blossom.INSTANCE.getConfigManager().save(args[1]).getValue());

                case "load":
                return ChatUtil.printInfo(Blossom.INSTANCE.getConfigManager().load(args[1]).getValue());
