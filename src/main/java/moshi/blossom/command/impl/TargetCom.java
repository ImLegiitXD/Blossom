...
extends Command {
    ...
    public String run(String[] args) {
        ...
        switch (args[0]) {
            ...
            case "add":
            if (args.length != 2)
            return ChatUtil.printInfo("Usage: .target add <name>");

            if (user.targetList.contains(args[1]))
            return ChatUtil.printInfo("\"" + args[1] + "\" is already on your target list");
            ...
            case "remove":
            if (args.length != 2)
            return ChatUtil.printInfo("Usage: .target remove <name>");

            if (!user.targetList.contains(args[1]))
            return ChatUtil.printInfo("\"" + args[1] + "\" is not on your target list");
