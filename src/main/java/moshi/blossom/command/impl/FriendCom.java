...
extends Command {
    ...
    public String run(String[] args) {
        ...
        switch (args[0]) {
            ...
            case "add":
            if (args.length != 2)
            return ChatUtil.printInfo("Usage: .friend add <name>");

            if (user.friendList.contains(args[1]))
            return ChatUtil.printInfo("\"" + args[1] + "\" is already on your friend list");
            ...
            case "remove":
            if (args.length != 2)
            return ChatUtil.printInfo("Usage: .friend remove <name>");

            if (!user.friendList.contains(args[1]))
            return ChatUtil.printInfo("\"" + args[1] + "\" is not on your friend list");
