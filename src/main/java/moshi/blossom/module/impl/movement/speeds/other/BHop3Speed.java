...
public class BHop3Speed extends ModuleMode {
    ...
    public BHop3Speed() {
        ...
        this.handleMove = (event -> {
            ...
            if ((event.getPlayer()).onGround) {
                ...
                if (this.airTicks >= 8) TimerUtil.setTimer(1.0F);
            ...
            if (this.veloTicks == 1) this.motion = handleVelo(this.veloMotion);

            if ((getPlayer()).onGround || (this.veloTicks < 8 && this.veloTicks > 2)) {
    ...
    private double modifier() {
        if (ModManager.getMod("Scaffold").isToggled()) return 0.99D;
    ...
    private double handleVelo(double motion) {
        if (motion > this.motion) motion *= 0.5D;
    ...
    private boolean shouldFix() {
        ...
        return (MoveUtil.isMoving() && (!aura.isToggled() || aura.target == null || getPlayer().getDistanceToEntity(aura.target) >= 3.5D) && !ModManager.getMod("Scaffold").isToggled());
